...
class _PetSettingsScreenState extends State<PetSettingsScreen> {
  ...
  late final TextEditingController _idleTransparentController;
  ...
  void initState() {
    ...
    _idleTransparentController = TextEditingController();
    _loadConfig();
  }
  ...
  void dispose() {
    ...
    _idleTransparentController.dispose();
    super.dispose();
  }
  ...
  Future<void> _loadConfig() async {
    ...
    _idleTransparentController.text = _config.idleTransparentMinutes.toString();
  ...
  Widget build(BuildContext context) {
    if (!_loaded) {
      return Scaffold(
        ...
        body: Column(children: List.generate(6, (i) => const ShimmerCard(lines: 2))),
      );
    }
    return Scaffold(
      appBar: AppBar(title: const Text('🐾 弗糯糯设置')),
  ...
  Widget _buildIdleTransparent() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('🫥 空闲透明模式', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500)),
        const SizedBox(height: 4),
        Text('宠物空闲指定分钟数后进入半透明（30%不透明度），3连击手动进入/退出',
            style: TextStyle(fontSize: 12, color: Theme.of(context).colorScheme.onSurfaceVariant.withValues(alpha: 0.6))),
        const SizedBox(height: 12),
        TextField(
          controller: _idleTransparentController,
          keyboardType: TextInputType.number,
          style: const TextStyle(fontSize: 14),
          decoration: const InputDecoration(
            labelText: '空闲超时（分钟）',
            hintText: '输入 1-120 之间的数字',
            suffixText: '分钟',
          ...
          onSubmitted: (v) {
            ...
            if (n != null && n >= 1 && n <= 120) {
              ...
              petOverlayController.syncTransparentIdle(n);
            } else {
              // 输入无效 → 恢复当前值
              _idleTransparentController.text = _config.idleTransparentMinutes.toString();
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('请输入 1-120 之间的数字'), duration: Duration(seconds: 1)),
                );
              }
            }
          },
